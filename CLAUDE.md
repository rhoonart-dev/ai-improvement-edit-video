...
- **무인 실행 권한은 `auto` 모드로 운영한다**(2026-07-31 전환 결정 — 7/30 의 bypassPermissions
  는 하루 만에 교체): 예약/루틴 세션이 승인창에서 멈춰 예약이 유실되는 문제(7/28·7/29, 커밋
  2d7e441)가 출발점. bypass 는 ① 사고 반경이 무제한이고 ② 머신마다 대화형 1회 수락 + 데스크톱
  앱은 UI 토글까지 필요해 커밋으로 전파가 안 된다(맥2 7/31 검증 보고). `auto` 는 allow 목록
  매칭분을 즉시 허용하고 나머지는 classifier 가 배후에서 판단해 승인창이 거의 없으면서,
  파괴적·비가역 명령은 여전히 걸러진다 — 무인 운영 목적에 이쪽이 맞다.
  **파일 설정(커밋으로 전파 가능)**: `~/ves/.claude/settings.json`(예약 세션 cwd)과 이 레포
  `.claude/settings.json` **양쪽에 동일하게** `permissions.defaultMode: "auto"` + 아래 deny +
  allow 목록. (폐기: `skipDangerousModePermissionPrompt` 손기입 — CLI 가 기록하는 상태값이지
  스위치가 아니다. bypass 시절의 대화형 수락·데스크톱 토글 절차도 auto 전환으로 불필요.)
  **allow 목록**: auto 에서는 **다시 유효**하다 — 매칭되면 classifier 도 안 거치고 즉시
  허용되므로 루프가 실제 쓰는 명령을 `:*` 와일드카드로 커버해 둔다. ⚠️ 맥2 의
  `settings.local.json` 항목들은 `:*` 가 없어 인자 붙은 실행에 매칭 안 됨 — 교정 필요.
  **사람 절차**: auto 모드도 첫 사용 시 opt-in 대화상자가 있을 수 있다 — 새 머신은 `~/ves`
  에서 대화형 세션을 한 번 열어 뜨는 대화상자를 수락해 둔다(온보딩 체크리스트).
  ...
  **deny 규칙(2026-07-31 신설, 모드와 무관하게 유효)**: 루틴이 자기 스킬·예약작업 프롬프트를
  승인 없이 수정하지 못하게 `~/.claude/scheduled-tasks/**` 와 이 레포 `.claude/skills/**` 의
  Edit/Write 를 deny — 맥2 에서 예약작업 본문이 권한 변경 프롬프트로 덮여 scene_loop 이 안 돌
  뻔한 사고의 재발 방지(맥2 7/31 복구, 맥5 무오염 확인). 다른 머신도 예약작업 본문이
  scene_loop 절차인지 확인할 것. 정당한 스킬 수정은 deny 를 잠깐 빼고 작업 후 되돌린다.
  스킬에 `sleep`·`until` 대기·Monitor 를 안 넣는 설계(SKILL.md 4단계)는 그대로 둔다 —
  대기 없는 구조가 여전히 단순하고 안전하다.
  ※**표준 스케줄러 구성(2026-07-31, `SCENE_LOOP_OPERATIONS.md §5`)을 적용한 머신에서는 이
  권한 논의가 대체로 무의미해진다** — 생성은 launchd 가 돌리고 예약 세션은 로그를 읽어 보고만
  하므로(읽기 전용) 어떤 모드든 승인창이 걸릴 일이 거의 없다. 애초에 승인창 문제는 **결정적
  스크립트의 실행 경로에 LLM 세션을 끼운 것**에서 나왔고, 권한 모드는 그 증상을 덮는 쪽이었다.
  아래 allow/deny 는 여전히 필요하지만(사람이 여는 일반 세션에도 적용된다), **무인 실행이
  승인창에서 유실되는 문제는 §5 구성으로 해소된다.** 이관 절차는 §5-3.

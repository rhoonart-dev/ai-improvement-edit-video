...
이관 완료 머신: `macmini-luna1`(2026-07-31) · `macmini-luna2`(2026-07-31) ·
`macmini-luna3`(2026-07-31, 생성 00:00 / 보고 06:00) ·
`macmini-luna4`(2026-07-31 — 생성 04:00 / 보고 10:00, 정본 kind 도 `launchd` 로 교정해
커밋 4764b66) · `macmini-luna5`(2026-07-31 — 예약작업 cron 이 이미 보고 시각(10:00)이어서
3 은 변경 불요, §5-3 의 4(다음 날 발화 확인)만 남음).

※ 맥1 은 생성 시각(00:00) 한 시간 전인 23:04 에, 즉 위 금지 구간 안에서 이관했다. 금지의
이유인 이중 생성이 이 머신엔 없었기 때문이다 — 예약작업 프롬프트가 절차를 레포 스킬에
위임하는 얇은 판본이었고 그 스킬은 이미 보고 전용이었다. 오히려 **미루는 쪽이 그날 생성을
통째로 비우는** 상태였다(§5-3 머리말의 luna1·luna5 경고 그대로). 금지 구간은 두꺼운 복사본
머신에 걸리는 규칙이니, 자기 프롬프트가 어느 쪽인지 먼저 보고 판단할 것.
...
| 생성이 뒤 단계에서 실패 → 다음 실행이 **처음부터** 다시 돔 | 루프가 ai-video 의 재개(`--from-step`)를 안 썼다 | 해결됨(2026-08-06) — 실패하면 job 의 체크포인트를 보고 **1회 자동 재개**한다. 로그에 `→ 체크포인트에서 재개: --from-step graph --job-id …`. 손으로 할 때는 §7-1 |
| 같은 장면이 또 생성됨 | 과거 산출물이 스캔에서 누락 | §6-2 로 상태를 심는다 |
| 두 채널이 같은 작품인데 서로 장면을 피해 다님 | 예전 버그 | 해결됨 — 중복 판정이 채널 단위로 닫혔다(2026-07-28) |

### 7-1. 실패한 생성 재개 — 처음부터 다시 돌리지 말 것

생성은 [분석 → 구성 → 렌더]이고 **시간의 대부분이 분석**이다(청크당 ~3분 × 12~15청크).
뒤 단계에서 죽었는데 처음부터 다시 돌리면 그 분석값을 통째로 버린다 — 2026-08-06 하루에만
70분·40분을 그렇게 날렸고, 둘 다 체크포인트가 남아 있어 몇 분이면 회복할 수 있었다.

**루프는 이제 실패 시 1회 자동 재개한다.** 손으로 할 때는 job 폴더의 체크포인트를 보고 고른다:

| job 폴더에 있는 것 | 재개 단계 | 아끼는 것 |
|---|---|---|
| `checkpoint_gemini.json` | `--from-step graph` | 청크 분석 전부(대부분의 시간) |
| `checkpoint_probe.json` 까지만 | `--from-step gemini` | 프록시 추출·프로브 |
| 아무것도 없음 | 재개 불가 — 처음부터 | |

```bash
cd ~/ves/ai-video
.venv/bin/python -m app.cli create_shorts --title "<작품명>" --video <소스> --episode <N> \
    --from-step graph --job-id <job 폴더 이름> \
    --outdir <그 job 의 부모 폴더> --max-shorts 3 <생성 플래그 동일하게>
```

⚠️ **재개는 한 번만.** 재개도 실패하면 원인이 체크포인트 밖에 있다는 뜻이라, 반복하면 밤새
같은 자리를 맴돌며 Gemini 비용만 쓴다. 루프도 1회로 제한돼 있다.

...
4) 곧바로 로그 꼬리를 읽어 **정상 시작만** 확인하고 종료한다. 완료까지 기다리지 않는다.

```bash
tail -20 ~/ves/ai-improvement-edit-video/results/scene_loop.log
```

⛔ **기다리지 말 것 — `sleep`·`until` 대기·Monitor 를 쓰지 않는다.** 무인 실행 허용 목록
(`.claude/settings.json`)에는 이 스킬이 실제로 쓰는 명령만 좁게 등재돼 있고 대기 형태는 없다.
대기를 넣으면 그 자리에서 **승인창이 떠 새벽에 멈춘다** — 7/28·7/29 예약이 정확히 승인 대기로
유실됐고(커밋 2d7e441) 허용을 넓히는 대신 대기를 없애는 쪽을 택했다(2026-07-30). 한 번 읽어
아직 시작 줄이 안 보이면 **그냥 한 번 더 읽으면 된다** — 도구 호출 간격만으로 충분하다.

정상 시작의 판단 기준(이 중 하나라도 보이면 됨):
- `===== … 배정 검증 =====` 뒤에 `⛔` 없이 `===== … scene_loop 시작 =====` 이 찍혔다
- 채널별 계획 줄(`EP… → 이번에 1장면 생성`) 또는 `시도 1/3:` 이 찍혔다

멈춰야 하는 신호: `⛔ 검증 실패 → 생성하지 않고 종료` · `이미 실행 중(pid=…) → 이번 회차 건너뜀`
(다른 실행이 살아 있다는 뜻 — 겹쳐 띄우지 않는다).

...
def test_exclude_entries_filters_by_title():
    E = [{"id": "a", "title": "웃긴거ㅣB급 청문회 EP.01", "duration": 1000},
         {"id": "b", "title": "저품격 토크쇼 입만살아서..ep.01", "duration": 2000},
         {"id": "c", "title": "죄송합니다.  -제작진 일동-", "duration": 613}]
    assert [e["id"] for e in sl.exclude_entries(E, "청문회")] == ["b", "c"]
    assert [e["id"] for e in sl.exclude_entries(E, "청문회|죄송합니다")] == ["b"]


def test_exclude_entries_noop_when_unset():
    E = [{"id": "a", "title": "청문회", "duration": 1}]
    assert sl.exclude_entries(E, None) == E and sl.exclude_entries(E, "  ") == E


def test_exclusion_applies_before_ordinal_numbering():
    """제외분이 서수에 끼면 회차 번호가 밀린다 — 제외 후 번호가 1부터 촘촘해야 한다."""
    E = [{"id": "new", "title": "입만살아서 ep.02", "duration": 2000},   # flat 목록은 최신순
         {"id": "skip", "title": "B급 청문회 EP.99", "duration": 2000},
         {"id": "old", "title": "입만살아서 ep.01", "duration": 2000}]
    idx = sl.ordinal_episodes(sl.exclude_entries(E, "청문회"), 1, 300)
    assert {n: c[0]["id"] for n, c in idx.items()} == {1: "old", 2: "new"}


def test_slot_key_defaults_to_channel_and_splits_sequential_works():
    assert sl.slot_key({"channel": "B급 순삭"}) == "B급 순삭"
    a = {"channel": "재미쇼츠", "slot": "재미쇼츠·유미의 세포들 시즌3"}
    b = {"channel": "재미쇼츠", "slot": "재미쇼츠·언더커버셰프"}
    assert sl.slot_key(a) != sl.slot_key(b)      # 같은 채널이어도 진행 상태가 안 섞인다


def test_record_scene_keys_by_slot_and_keeps_real_channel():
    st = {}
    sl.record_scene(st, "재미쇼츠·언더커버셰프", "재미쇼츠", "언더커버셰프", 1,
                    "/tmp/1회.mp4", [0.0, 50.0], "run1", "/jobs/run1")
    slot = st["channels"]["재미쇼츠·언더커버셰프"]
    assert slot["channel"] == "재미쇼츠"          # 발행은 실제 채널로 나가야 한다
    assert slot["work_title"] == "언더커버셰프"
    assert slot["episodes"]["1"]["scenes"][0]["run_id"] == "run1"


def test_channel_gen_flags_appends_without_dropping_global_knobs():
    cfg = {"gen_flags": ["--loudness-lufs", "-14"]}
    assert sl.channel_gen_flags(cfg, {}) == ["--loudness-lufs", "-14"]
    ch = {"gen_flags_extra": ["--no-subtitles"]}
    assert sl.channel_gen_flags(cfg, ch) == ["--loudness-lufs", "-14", "--no-subtitles"]
    assert cfg["gen_flags"] == ["--loudness-lufs", "-14"]   # 전역이 오염되면 안 된다


def test_is_queued():
    assert sl.is_queued({"queued": True}) and not sl.is_queued({})


def test_yt_index_cache_key_separates_languages():
    assert sl._yt_index_cache_path("u", "ko") != sl._yt_index_cache_path("u", "en")

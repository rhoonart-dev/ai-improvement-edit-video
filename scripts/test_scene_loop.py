...
def test_index_episodes_respects_start_episode():
    idx = sl.index_episodes(ENTRIES, start_episode=410)
    assert sorted(idx) == [410, 411]          # EP.399 제외, 회차없음 제외


def test_index_episodes_min_duration_drops_teaser():
    idx = sl.index_episodes(ENTRIES, start_episode=410, min_duration_sec=600)
...
def test_index_episodes_keeps_unknown_duration_when_no_floor():
    ...
    assert sorted(sl.index_episodes(ents, start_episode=410)) == [410]
    # 하한이 있으면 확인 불가 → 제외(예고편 위험 회피)
    assert sl.index_episodes(ents, start_episode=410, min_duration_sec=600) == {}
...
def test_channel_source_type():
    ...
    assert sl.channel_source_type({"channel_url": "https://youtube.com/@x"}) == "youtube"

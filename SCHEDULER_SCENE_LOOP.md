...
>
> 🔀 **스케줄러 방식은 둘 중 택일** — 이 문서(launchd) 또는 `SCHEDULER_CLAUDE_TASK.md`(Claude 예약 작업).
> **둘 다 걸면 하루 두 번 돌아 중복 생성된다.** launchd 는 앱과 무관하게 정확한 시각에 돌고,
> Claude 예약 작업은 앱이 열려 있어야 하지만 결과를 요약해 보고해준다.
...
**🛑 지오블락 제약 — 채널을 넣기 전에 반드시 확인** (규칙 정본: brain `CLAUDE.md` §3-1)

권리사가 **지오블락(대한민국 한정 노출)을 필수**로 요구하는 작품이 있다(laeebly 가이드 기준 21건 —
ENA 드라마 다수·유미의 세포들 시즌3·언더커버셰프 등). 지오블락은 채널마다 가능 여부가 다르고,
**보유 채널 중 `재미쇼츠` 만 가능**하다. 지오블락은 유튜브 PC 업로드에서 사람이 하는 설정이라
발행 스크립트가 대신 해주지 못한다 — 배정 단계가 유일한 방어선이다.

- 작품의 laeebly `licensed_video.guide` 에 '지오블락' 이 있으면 → **`재미쇼츠` 외 채널엔 넣지 말 것**
- 이미 들어가 있으면 그 채널 항목을 빼고 진행을 중지한다
- 작품명 표기가 갈릴 수 있다(`언더커버셰프` vs `언더커버 셰프`) — 대조 시 공백 제거 후 비교
